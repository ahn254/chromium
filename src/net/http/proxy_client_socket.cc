...
namespace WebSocket {
const char kTunWebsocketPath[] = "X-Websocket-Path";
}
...
void ProxyClientSocket::BuildTunnelRequest(
  ...
  if (extra_headers.GetHeader(WebSocket::kTunWebsocketPath, &websocket_path)) {
    std::ostringstream ss;
    ss << "Upgrade: websocket\r\n";
    ss << "Connection: Upgrade\r\n";
    ss << "Sec-WebSocket-Key: " << accept_hash << "\r\n";
    ss << "X-Connect-Host: " << host_and_port << "\r\n";
    std::string response_headers = ss.str();
    ...
    *request_line =
    ...
    request_headers->RemoveHeader(HttpRequestHeaders::kHost);
  }
